...
function Alunos() {
    ...
    useEffect(() => {
        try {
            ...
            axios.get(`${env.apigatewayAP}/utilizadores?type=0&token=${localStorage.token}`)
                .then(response => {
                    setUsers(response.data)
                })
                .catch(error => {
                    console.log(error)
                    toast.error("Não foi possível obter a lista de alunos!", {
    ...
    const handleFileChange = (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const filteredJson = json.filter(item => item.type === 0);
                setJsonContent(filteredJson);

                axios.post(`${env.apigatewayAP}/utilizadores/registerFile?token=${localStorage.token}`, filteredJson)
                    .then((response) => {
                        toast.success("Os alunos foram adicionados com sucesso!", { position: toast.POSITION.TOP_CENTER });
                    })
                    .catch((error) => {
                        toast.error("Não foi possível adicionar os alunos!", { position: toast.POSITION.TOP_CENTER });
                    });
    ...
    const handleSubmit = (event) => {
        ...
        axios.post(`${env.apigatewayAP}/utilizadores/register?token=${localStorage.token}`, form)
            .then((response) => {
                toast.success("O aluno foi adicionado com sucesso!", { position: toast.POSITION.TOP_CENTER })
            })
            .catch((error) => {
                toast.error("Não foi possível adicionar o aluno!", { position: toast.POSITION.TOP_CENTER })
            })
                        ...
                        <Card.Body>
                            ...
                            {showFicheiro &&
                                <Container className="my-4 mb-5">
                                    <h4 className="mb-3">Adicionar Alunos</h4>
                            ...
                            {showAdd &&
                                <Form onSubmit={handleSubmit}>
                                    <Container className="my-4 mb-5">
                                        <h4 className="mb-3">Adicionar Aluno</h4>
                                        ...
                                        <Row className="gx-3 mb-3">
                                            ...
                                            <Col md={6}>
                                                <div className="d-flex justify-content-left mt-2">
                                                    <Button type="submit" className="mx-2" variant="outline-dark" startIcon={<Check />}>Adicionar Aluno</Button>
                                                </div>
                                            </Col>
                                        </Row>
                                    </Container>
                            ...
                            <Container className="my-4"><h4>Alunos</h4></Container>
                                                ...
                                                {decodedToken.type === 2 &&
                                                    <>
                                                        ...
                                                        <Modal show={showModal && user._id === deleteNumMec} onHide={handleHideModal}>
                                                            ...
                                                            <Modal.Body>
                                                                <div className="alert alert-danger">Tem a certeza que pretende remover este aluno?</div>
                                                            </Modal.Body>
                                                            <Modal.Footer>
